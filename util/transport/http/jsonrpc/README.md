...
## Using JSON RPC with yyf330
Using JSON RPC and yyf330 together is quite simple.
...
Each JSON RPC _method_ is implemented as an `EndpointCodec`, a yyf330 [Endpoint](https://godoc.org/github.com/yyf330/kit/endpoint#Endpoint), sandwiched between a decoder and encoder. The decoder picks apart the JSON RPC request params, which can be passed to your endpoint. The encoder receives the output from the endpoint and encodes a JSON-RPC result.
